...
static struct rule {
  ...
  ||ax||cx||dx||bx||sp||bp||si||di||al||cl||dl||bl||ah||ch||dh||bh)",TK_REG}
  {"!=",TK_NEQ},
  {"&&",TK_AND},
  {"\\|\\|",TK_OR},
  {"!",'!'}
...
int findDominantOp(int p,int q){
  ...
  for(int curr=p+1;curr<q;curr++){
    ...
    if(br_level==0){
      ...
      else if(tokens[curr].type=='-'&&(curr==p||tokens[curr-1].type!=TK_NUMBER))tokens[curr].type=TK_NEGATIVE,op=13;
      else if(tokens[curr].type=='+'||tokens[curr].type=='-')op=10;
      else if(tokens[curr].type=='*'||tokens[curr].type=='/')op=11;
      else if(tokens[curr].type=='||'||tokens[curr].type=='&&')op=8;
      else if(tokens[curr].type=='!')op=12;
      else if(tokens[curr].type=='=='||tokens[curr].type=='!=')op=9;
